  ...
  const allChoices = useMemo(() => {
    ...
    data.data.forEach((q) => {
      q.responses.forEach((r) => {
        if (r.percentage > 0 && !map.has(r.choice)) {
          map.set(r.choice, {
            key: r.choice,
            label: r.label,
            color: colorByKey[r.choice] ?? "#BDBDBD",
          });
        }
      });
    });
  ...
  useEffect(() => {
    ...
    data.data.forEach((q, i) => {
      const g = root
        ...
        .attr(
          "transform",
          `translate(0, ${headerFontSize + headerGap + i * (rowH + gap)})`,
        );
      // Titre question
      g.append("text")
        .attr("x", 0)
      ...
      segments.forEach((r, idx) => {
        ...
        const fillColor =
          allChoices.find((c) => c.key === r.choice)?.color ?? light1;
        g.append("rect")
          .attr("x", x)
          ...
          .on("mousemove", function (event: MouseEvent) {
            ...
            tooltipSel
              ...
              .style("opacity", 1)
              .text(
                `${q.emoji ?? "🎯"} ${q.title} — ${r.label}: ${r.percentage.toFixed(1)}%`,
              );
          })
