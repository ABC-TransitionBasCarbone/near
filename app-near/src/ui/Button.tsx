...
const Button = ({
  ...
  return (
    <button
      ...
      type={buttonType}
      className={`
        flex gap-3 ${border ? 'border' : ''} ${colorVariants[style][color]} 
        ${rounded && 'rounded-md'} items-center justify-center px-5 py-2 font-sans font-bold 
        disabled:opacity-40 ${customStyle} 
        ${iconPosition === ButtonIconPosition.END && 'flex-row-reverse'} ${extend && 'w-full'} 
        ${responsive ? 'lg:gap-3 lg:p-3 lg:text-base md:gap-2 md:p-2 md:text-base sm:gap-2 sm:p-2 sm:text-xs gap-1 p-2 text-xs' : 'gap-3 p-3 '}
        ${iconRight ? 'flex-row-reverse justify-between' : ''}
        `}
      onClick={onClick}
      disabled={disabled}
