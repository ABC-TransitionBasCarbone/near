...
def extraction(i, ech):
    ...
    df["C"] = df["C"].apply(
        lambda x: 1 if x == "Mobilités actives (vélo, marche" else x
    )
    ...
    df["G"] = df["G"].apply(
        lambda x: 1 if x == "électricité, réseau de chaleur," else x
    )
...
def Hclustering(rows, Floor, Top, Mincount, distance=eucl):
    ...
    while len(clust) > Mincount and lowestweight < Floor and biggestweight < Top:
        ...
        for i in range(len(clust)):
            for j in range(i + 1, len(clust)):
                ...
                if (clust[i].ide, clust[j].ide) not in distances:
                    distances[(clust[i].ide, clust[j].ide)] = distance(
        ...
        for item in lowestpair:
            ...
            newcluster = cluster(
                vec=mergevec, fils=fils, distance=closest, ide=currentclustid, poids=M
            )
